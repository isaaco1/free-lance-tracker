# Freelancer Timer v4 - Added Minimum Billable Time logic
...
class TimerApp:
    def __init__(self, root):
        ...
        self.min_var = tk.StringVar(value="6")
        ...
        self.pause_button.grid(row=5, column=1, padx=5, pady=10, sticky="w")

        self.stop_button = ttk.Button(root, text="Stop", command=self.stop_timer, state="disabled")
        self.stop_button.grid(row=5, column=1, padx=5, pady=10, sticky="e")
        ...
        if not os.path.exists("sessions.csv"):
            with open("sessions.csv", "w", newline="") as f:
                ...
                writer.writerow([
                    ...
                    "total_duration_min", "billable_min",
                    "rate_gbp", "earned_gbp"
                ])
    ...
    def stop_timer(self):
        if not self.running:
            return
        total_seconds = int(time.time() - self.start_time) + self.elapsed_seconds if not self.paused else self.elapsed_seconds
